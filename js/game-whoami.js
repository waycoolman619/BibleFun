...
const WhoAmIGame = {
            ...
            <div class="global-settings">
                <div class="setup-row setup-row-single">
                    <div class="setup-card">
                        <span class="setup-card-label">How many characters?</span>
                        <div class="option-tiles" id="w-count-tiles">
                            <button type="button" class="option-tile selected" data-value="5" onclick="WhoAmIGame.pickCount(5)">5</button>
                            <button type="button" class="option-tile" data-value="10" onclick="WhoAmIGame.pickCount(10)">10</button>
                            <button type="button" class="option-tile" data-value="20" onclick="WhoAmIGame.pickCount(20)">20</button>
                            <button type="button" class="option-tile" data-value="99" onclick="WhoAmIGame.pickCount(99)">All</button>
                        </div>
                        <select id="w-count" aria-hidden="true" style="position:absolute;opacity:0;height:0;width:0"><option value="5" selected>5</option><option value="10">10</option><option value="20">20</option><option value="99">99</option></select>
                    </div>
                </div>
                <div class="battle-mode-card">
                    <span class="setup-card-label">Play mode</span>
                    <div class="option-tiles">
                        <button type="button" class="option-tile selected" id="w-solo-tile" onclick="WhoAmIGame.setPlayMode(false)">Solo</button>
                        <button type="button" class="option-tile" id="w-battle-tile" onclick="WhoAmIGame.setPlayMode(true)">Battle</button>
                    </div>
                    <input type="checkbox" id="w-battle-mode" onchange="WhoAmIGame.toggleBattleMode()" aria-hidden="true" style="position:absolute;opacity:0;height:0;width:0">
                    <div id="w-battle-options" class="battle-options-inner t-battle-options" style="display:none;">
                        <div class="setup-card" id="w-battle-type-card">
                            <span class="setup-card-label">Battle as</span>
                            <div class="option-tiles">
                                <button type="button" class="option-tile selected" onclick="WhoAmIGame.pickBattleType('individuals')">Individuals</button>
                                <button type="button" class="option-tile" onclick="WhoAmIGame.pickBattleType('teams')">Teams</button>
                            </div>
                            <select id="w-battle-type" aria-hidden="true" style="position:absolute;opacity:0;height:0;width:0"><option value="individuals">Individuals</option><option value="teams">Teams</option></select>
                        </div>
                        <div id="w-individuals-block" class="t-battle-block">
                            <div class="setup-card">
                                <span class="setup-card-label">Number of Players</span>
                                <div class="option-tiles" id="w-pcount-tiles">${[2,3,4,5,6].map((n) => `<button type="button" class="option-tile${n===2?' selected':''}" data-value="${n}" onclick="WhoAmIGame.pickPcount(${n})">${n}</button>`).join('')}</div>
                                <select id="w-pcount" onchange="WhoAmIGame.syncPcountFromSelect()" style="position:absolute;opacity:0;height:0;width:0">${Array.from({ length: 20 }, (_, i) => `<option value="${i + 1}"${i === 1 ? ' selected' : ''}>${i + 1}</option>`).join('')}</select>
                            </div>
                        ...
                        <div id="w-teams-block" class="t-battle-block" style="display:none;">
                            <div class="setup-card">
                                ...
                                <div class="option-tiles">
                                    <button type="button" class="option-tile selected" data-value="2" onclick="WhoAmIGame.pickTeamsCount(2)">2</button>
                                    <button type="button" class="option-tile" data-value="3" onclick="WhoAmIGame.pickTeamsCount(3)">3</button>
                                    <button type="button" class="option-tile" data-value="4" onclick="WhoAmIGame.pickTeamsCount(4)">4</button>
                                </div>
                                <select id="w-teams-count" aria-hidden="true" style="position:absolute;opacity:0;height:0;width:0"><option value="2" selected>2</option><option value="3">3</option><option value="4">4</option></select>
                            </div>
                            <div class="setting-item">
    ...
    pickCount: function(n) {
        const sel = document.getElementById('w-count');
        if (sel) sel.value = String(n);
        document.querySelectorAll('#w-count-tiles .option-tile').forEach(t => { t.classList.toggle('selected', parseInt(t.getAttribute('data-value'), 10) === n); });
    },
    setPlayMode: function(battle) {
        const cb = document.getElementById('w-battle-mode');
        if (cb) cb.checked = battle;
        if (document.getElementById('w-solo-tile')) document.getElementById('w-solo-tile').classList.toggle('selected', !battle);
        if (document.getElementById('w-battle-tile')) document.getElementById('w-battle-tile').classList.toggle('selected', battle);
        document.getElementById('w-battle-options').style.display = battle ? 'block' : 'none';
        if (battle) this.toggleBattleType();
    },
    pickBattleType: function(type) {
        const sel = document.getElementById('w-battle-type');
        if (sel) sel.value = type;
        const card = document.getElementById('w-battle-type-card');
    ...
    pickPcount: function(n) {
        const sel = document.getElementById('w-pcount');
        if (sel) sel.value = String(n);
        document.querySelectorAll('#w-pcount-tiles .option-tile').forEach(t => { t.classList.toggle('selected', parseInt(t.getAttribute('data-value'), 10) === n); });
        this.togglePlayerRows();
    },
    syncPcountFromSelect: function() {
        const n = parseInt(document.getElementById('w-pcount').value, 10);
        document.querySelectorAll('#w-pcount-tiles .option-tile').forEach(t => { t.classList.toggle('selected', parseInt(t.getAttribute('data-value'), 10) === n); });
        this.togglePlayerRows();
    },
    pickTeamsCount: function(n) {
        const sel = document.getElementById('w-teams-count');
        if (sel) sel.value = String(n);
        document.querySelectorAll('#w-teams-block .option-tiles .option-tile').forEach(t => { t.classList.toggle('selected', parseInt(t.getAttribute('data-value'), 10) === n); });
        this.toggleBattleType();
    },
    ...
    toggleBattleMode: function() {
        ...
        if (document.getElementById('w-solo-tile')) document.getElementById('w-solo-tile').classList.toggle('selected', !on);
        if (document.getElementById('w-battle-tile')) document.getElementById('w-battle-tile').classList.toggle('selected', on);
    ...
    start: function() {
        ...
        this.activeQuestions = pool.slice(0, Math.min(count, pool.length));
        this.currentIndex = 0;
